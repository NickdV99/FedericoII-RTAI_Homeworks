...
static void * wheel_loop(void * par) {
	...
	if (!(loop_Task = rt_task_init_schmod(nam2num("WHEEL")+i, 2+i, 0, 0, SCHED_FIFO, 0x1))) {
	...
	while (keep_on_running)
	{
		...
		if( shm_sudden_stop[i] ==  1  ){			//VERIFY IF A SUDDEN STOP OCCURRED
						
			if( sensor[i] > 50 ) sensor[i] = sensor[i] - 50;
			else sensor[i] = 0;	
...
int main(void)
{
	...
	shm_sudden_stop = rtai_malloc(SHM_STOP, NUM_OF_WHEELS * sizeof(int));
	
	int i;

	//INITIALIZATION OF THE SHM USED FOR SUDDEN STOP
